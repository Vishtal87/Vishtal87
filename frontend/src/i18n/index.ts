...
const ru = {
  ...
  from: 'С', to: 'По', apply: 'Применить', reset: 'Сбросить', close: 'Закрыть', clearSearch: 'Очистить поиск', back: 'Назад',
  ...
  updated: 'Обновлено',
  updatedAgo: (ago: string) => `обновлено ${ago}`,
  happened: 'Произошло',
  ...
  demoShort: 'ДЕМО',
  sourcesDown: (n: number, total: number) => `${n} из ${total} ${plural(total, 'источника', 'источников', 'источников')} временно недоступны`,
  loading: 'Загрузка…',
  error: 'Не удалось загрузить данные',
  retry: 'Повторить',
  nearCity: (name: string) => `рядом с «${name}»`,
  zoomHint: 'нажмите на точку или приблизьте регион',
  pulseTitle: 'Пульс планеты',
  pulseHour: 'событий за последний час',
  pulseDay: 'новых событий за сутки',
  pulsePlaces: (n: number) => `в ${n} ${plural(n, 'месте', 'местах', 'местах')} за сутки`,
  pulseSpark: 'Новые события по часам за сутки',
  pulseFilter: (name: string) => `Показать только: ${name}`,
  tickerNow: 'Сейчас',
...
const en: Dict = {
  ...
  from: 'From', to: 'To', apply: 'Apply', reset: 'Reset', close: 'Close', clearSearch: 'Clear search', back: 'Back',
  ...
  updated: 'Updated', updatedAgo: (ago) => `updated ${ago}`, happened: 'Happened', firstReport: 'First report', localTime: 'local time',
  ...
  sourcesDown: (n, total) => `${n} of ${total} sources temporarily unavailable`,
  loading: 'Loading…', error: 'Failed to load data', retry: 'Retry', nearCity: (name) => `near “${name}”`,
  pulseTitle: 'Pulse of the planet', pulseHour: 'events in the last hour', pulseDay: 'new events today',
  pulsePlaces: (n) => `in ${n} place${n === 1 ? '' : 's'} today`, pulseSpark: 'New events per hour, last 24 hours',
  pulseFilter: (name) => `Show only: ${name}`, tickerNow: 'Now',
  zoomHint: 'tap a hotspot or zoom into a region', latest: 'Latest events in view', theme: 'Theme', lang: 'Language', population: 'pop.',
