...
## 0. Быстрая установка одной командой

Подключитесь к серверу. В Windows откройте PowerShell, в macOS/Linux — Терминал:

```bash
ssh root@IP-сервера
```

Введите пароль (символы при вводе не отображаются, так и должно быть) и вставьте одну строку:
...
Скрипт:
- ставит Docker и скачивает код в `/opt/pulse`;
- сам создаёт `.env` со случайным паролем базы и подбирает параметры под объём памяти;
- собирает и запускает сайт, загружает все населённые пункты России, проверяет источники.

Установка идёт 20–40 минут в фоне: окно SSH можно закрыть, это её не прервёт. Следить за ходом:
`tail -f /var/log/pulse-install.log`. В конце выводятся адреса:

- `https://<IP-через-дефисы>.sslip.io`, например `https://203-0-113-10.sslip.io`. Покупать домен не нужно: имя
  sslip.io само указывает на ваш IP, и HTTPS-сертификат выпускается автоматически;
- `http://<IP>` — запасной вход без шифрования, работает сразу.

Если порты 80/443 уже заняты другим сервисом (другой сайт, панель, VPN), установщик его не трогает: сайт
поднимается на свободном порту, обычно 8080, и адрес будет `http://<IP>:8080`. HTTPS в этом режиме
невозможен: для сертификата Let's Encrypt нужны порты 80/443.

Чтобы в этом режиме был HTTPS, пусть уже работающий на сервере веб-сервер отдаёт наш сайт на своём домене
(или поддомене, A-запись которого указывает на сервер). Caddy:

```
news.example.ru {
    reverse_proxy 127.0.0.1:8080 {
        flush_interval -1        # живые обновления (SSE) без задержки
    }
}
```

nginx: `location / { proxy_pass http://127.0.0.1:8080; proxy_set_header Host $host; proxy_buffering off; }`.
Если этот веб-сервер сам работает в Docker, вместо `127.0.0.1` укажите IP сервера. Наш сайт принимает любое
имя по HTTP, поэтому отдельно его настраивать не нужно.

Повторный запуск той же команды безопасен: данные и `.env` сохраняются, код обновляется, выполненные шаги
пропускаются.

Установщик включает **автообновление**: раз в 30 минут сервер проверяет ветку в GitHub. Если есть новые
коммиты, он пересобирает и перезапускает сайт. Если после этого сайт не отвечает 3 минуты, сервер
возвращается на прежнюю версию и больше не пробует этот коммит. Журнал: `journalctl -u pulse-autoupdate`.
Выключить: `systemctl disable --now pulse-autoupdate.timer`. Или ставьте сразу без него:
`… | AUTOUPDATE=0 bash`. Учтите: всё, что попадает в ветку, само приезжает на сервер. Давайте права на запись
в репозиторий только тем, кому доверяете сервер.

Свой домен: перед запуском добавьте его в команду: `… | DOMAIN=news.example.ru bash`. A-запись домена должна
указывать на сервер (раздел 2).

После установки смените пароль root командой `passwd`, а лучше настройте вход по SSH-ключу.

Разделы ниже описывают то же самое вручную, по шагам.
...
- `DOMAIN` — ваш домен (`news.example.ru`) или `<IP-через-дефисы>.sslip.io`;
- `PUBLIC_IP` — IP сервера (сайт будет доступен и по `http://IP`);
...
| `scripts/prod.sh health` | код 0, если сайт отвечает через Caddy на этой машине |
| `systemctl list-timers pulse-autoupdate.timer` | когда будет следующая проверка обновлений |
...
|---|---|
| Сайт не открывается, в `logs caddy` ошибки сертификата | A-запись указывает на сервер? Порты 80/443 открыты у провайдера и в `ufw status`? Пока сертификата нет, сайт доступен по `http://IP` |
| `Bind for 0.0.0.0:80 failed: port is already allocated` | Порт 80 занят другим контейнером (`docker ps --filter publish=80`). Запустите установку ещё раз: она переведёт сайт на свободный порт (`HTTP_PORT` в `.env`) |
